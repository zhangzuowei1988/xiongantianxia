...
- (void)setup{
    ...
    //slideView_.backgroundColor=[UIColor purpleColor];
...
- (UIViewController *)DLSlideView:(DLSlideView *)sender controllerAt:(NSInteger)index{
   // NSString *key = [NSString stringWithFormat:@"%ld", (long)index];
    ...
    // DLScrollTabbarView *bar=self.tabbar;
    
   
    
    
    PDMITagItem *item=[self.tabbar.tabbarItems objectAtIndex:index];
    NSString *key=item.columnId;
...
{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(showOrHiddenAddChannelsCollectionView:)]) {
...
-(void)editBtnPressed
{
   // tagView.inEditState=YES;
    if (self.delegate&&[self.delegate respondsToSelector:@selector(editBtnPressed)]) {
        [self.delegate editBtnPressed];
    }
}
-(void)editCompletedPressed
{
   // tagView.inEditState=NO;
    if (self.delegate &&[self.delegate respondsToSelector:@selector(editCompletedPressed)]) {
