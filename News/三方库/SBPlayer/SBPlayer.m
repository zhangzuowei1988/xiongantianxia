...
//原始约束
@property(nonatomic,strong)NSArray *oldConstraints;
...
-(void)setThumbImageStr:(NSString *)thumbImageStr{
    
    __weak SBPlayer *sb_Player=self;
    if (_thumbImageStr!=thumbImageStr) {
        _thumbImageStr=thumbImageStr;
        self.pauseOrPlayView.thumbImageView.hidden=NO;
        [sb_Player.pauseOrPlayView.thumbImageView sd_setImageWithURL:[NSURL URLWithString:thumbImageStr] placeholderImage:[CommData shareInstance].placeholderImage completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
...
-(void)assetWithURL:(NSURL *)url{
    ...
    self.controlView.totalTime = @"00:00";
     ...
     __weak SBPlayer *sb_Player=self;
    [sb_Player.anAsset loadValuesAsynchronouslyForKeys:keys completionHandler:^{
        NSError *error = nil;
        AVKeyValueStatus tracksStatus = [sb_Player.anAsset statusOfValueForKey:@"duration" error:&error];
        switch (tracksStatus) {
            case AVKeyValueStatusLoaded:
            {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (!CMTIME_IS_INDEFINITE(sb_Player.anAsset.duration)) {
                        CGFloat second = sb_Player.anAsset.duration.value / sb_Player.anAsset.duration.timescale;
                        sb_Player.controlView.totalTime = [sb_Player convertTime:second];
                        sb_Player.controlView.minValue = 0;
                        sb_Player.controlView.maxValue = second;
                        //‣self.pauseOrPlayView.timeLabel.text=[sb_Player convertTime:second];
                        
                        self.pauseOrPlayView.timeLabel.text=[NSString stringWithFormat:@"%@ %@",@"▶︎",[sb_Player convertTime:second]];
...
-(void)deviceOrientationDidChange:(NSNotification *)notification{
    ...
    switch (_interfaceOrientation) {
        ...
        {
            _isFullScreen = YES;
            if (!self.oldConstraints) {
                UIView *superView=(UIView*)[self nextResponder];
                self.oldView=superView;
                self.oldConstraints = superView.constraints;
           ...
           // [self removeFromSuperview];
            ...
            [UIView animateKeyframesWithDuration:0.5 delay:0 options:UIViewKeyframeAnimationOptionCalculationModeLinear animations:^{
                ...
                if (self.oldConstraints) {
                    ...
                    [self.oldView  addConstraints:self.oldConstraints];
                }
//                [self mas_updateConstraints:^(MASConstraintMaker *make) {
//                   
...
-(UILabel *)titleLabel{
    if (!_titleLabel) {
        ...
        _titleLabel.font = [UIFont systemFontOfSize:FirstTitleFontSize*[CommData shareInstance].scale];
...
-(void)play{
    ...
    if (self.pauseOrPlayView.thumbImageView.hidden==NO) {
        self.pauseOrPlayView.thumbImageView.hidden=YES;
    }
